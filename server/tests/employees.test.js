...
describe('GET /api/employees', () => {
  ...
  test('honours page and pageSize', async () => {
    const res = await api('get', '/api/employees?page=2&pageSize=5');
  ...
  test('page 1 and page 2 return different records', async () => {
    const first = await api('get', '/api/employees?page=1&pageSize=5');
    const second = await api('get', '/api/employees?page=2&pageSize=5');
  ...
  test('filters by department', async () => {
    const res = await api('get', '/api/employees?departmentId=1');
  ...
  test('search matches across name, email, code and position', async () => {
    const res = await api('get', '/api/employees?search=cruz');
  ...
  describe('query parameter validation', () => {
    const badRequests = [
      ['negative pageSize', '/api/employees?pageSize=-5'],
      ['negative page', '/api/employees?page=-1'],
      ['zero page', '/api/employees?page=0'],
      ['pageSize above the cap', '/api/employees?pageSize=5000'],
      ['non-numeric departmentId', '/api/employees?departmentId=abc'],
      ['unknown status', '/api/employees?status=Bogus'],
      ['unknown sort column', '/api/employees?sortBy=DROP%20TABLE'],
      ['invalid sort order', '/api/employees?sortOrder=sideways'],
    ];
    ...
    test('treats cleared filters as absent rather than invalid', async () => {
      const res = await api('get', '/api/employees?departmentId=&status=&search=');
      assert.equal(res.status, 200);
    });
  });
});
...
describe('GET /api/employees/:id', () => {
  test('returns a single employee', async () => {
    const list = await api('get', '/api/employees?pageSize=1');
  ...
  test('returns 404 for an id that does not exist', async () => {
    const res = await api('get', '/api/employees/99999999');
    assert.equal(res.status, 404);
  });

  test('returns 400 for a non-numeric id', async () => {
    const res = await api('get', '/api/employees/not-a-number');
...
describe('POST /api/employees', () => {
  ...
  test('the created record is retrievable afterwards', async () => {
    const created = await api('post', '/api/employees', buildEmployee());
    const fetched = await api('get', `/api/employees/${created.body.employeeId}`);
  ...
  test('rejects a duplicate employee code with 409', async () => {
    const first = await api('post', '/api/employees', buildEmployee());

    const clash = buildEmployee({ employeeCode: first.body.employeeCode });
    const res = await api('post', '/api/employees', clash);
  ...
  test('rejects a department that does not exist with 400', async () => {
    const res = await api('post', '/api/employees', buildEmployee({ departmentId: 99999 }));
  ...
  for (const [label, override] of invalidFields) {
    test(`rejects ${label} with 400`, async () => {
      const res = await api('post', '/api/employees', buildEmployee(override));
...
describe('PUT /api/employees/:id', () => {
  test('updates a record and returns the new values', async () => {
    ...
    const res = await api('put', `/api/employees/${created.body.employeeId}`, updated);
  ...
  test('returns 404 for an id that does not exist', async () => {
    const res = await api('put', '/api/employees/99999999', buildEmployee());

    assert.equal(res.status, 404);
  });

  test('rejects taking an email already used by another record with 409', async () => {
    const a = await api('post', '/api/employees', buildEmployee());
    const b = await api('post', '/api/employees', buildEmployee());

    const res = await api(
      'put',
      `/api/employees/${b.body.employeeId}`,
  ...
  test('rejects an invalid body with 400', async () => {
    ...
    const res = await api(
      'put',
      `/api/employees/${created.body.employeeId}`,
...
describe('DELETE /api/employees/:id', () => {
  test('deletes a record and responds 204 with no body', async () => {
    ...
    const res = await api('delete', `/api/employees/${created.body.employeeId}`);
  ...
  test('the record is gone afterwards', async () => {
    const created = await api('post', '/api/employees', buildEmployee());
    await api('delete', `/api/employees/${created.body.employeeId}`);

    const res = await api('get', `/api/employees/${created.body.employeeId}`);
    assert.equal(res.status, 404);
  });
  ...
  test('deleting the same record twice returns 404', async () => {
    ...
    const first = await api('delete', `/api/employees/${created.body.employeeId}`);
    const second = await api('delete', `/api/employees/${created.body.employeeId}`);
...
describe('GET /api/departments', () => {
  test('returns the department lookup list', async () => {
    ...
    assert.ok(res.body[0].DepartmentId);
    assert.ok(res.body[0].DepartmentName);
  });
});
